...
`kten` intentionally exposes only a focused subset. Card creation is its only
write operation; all other commands are read-only:
...
Card creation is the only supported Kaiten write operation. `kten` does not
